# TradingAgents 專案提示詞彙整

本文檔包含專案中所有與 AI 溝通的提示詞，完全保持原始內容不變。

## 提示詞結構說明

### System Message 變數
在分析師團隊中，每個分析師都有一個 `system_message` 變數，定義了該角色的特定任務和職責。這個 `system_message` 會被插入到 Prompt Template 中，與基礎的協作提示詞組合形成完整的系統提示詞。

### Prompt Template 結構
Prompt Template 通常包含：
1. **基礎協作提示詞**：定義 AI 助理的協作行為
2. **{system_message} 變數**：會被替換為具體角色的 System Message
3. **動態變數**：如 `{tool_names}`, `{current_date}`, `{ticker}` 等

### 使用方式
在程式碼中，`system_message` 變數會通過 `prompt.partial(system_message=system_message)` 的方式填入到 Prompt Template 中，形成最終發送給 LLM 的完整提示詞。

---

## 一、分析師團隊

### 1. 市場分析師 (Market Analyst)

**檔案位置**: `tradingagents/agents/analysts/market_analyst.py`

#### System Message (system_message 變數)
```
...
#### Prompt Template 完整結構 (System Role)
```
...
**說明**: 此 Prompt Template 會將 `{system_message}` 變數替換為上面的 System Message 內容，形成完整的系統提示詞。

---

### 2. 社交媒體分析師 (Social Media Analyst)
...
### 9. 積極風險分析師 (Risky Risk Analyst)

**檔案位置**: `tradingagents/agents/risk_mgmt/aggresive_debator.py`
...
### 10. 保守風險分析師 (Safe/Conservative Risk Analyst)
...
### 11. 中性風險分析師 (Neutral Risk Analyst)

**檔案位置**: `tradingagents/agents/risk_mgmt/neutral_debator.py`
...
### 12. 風險評估官 (Risk Management Judge)
...
### 13. 信號處理器 (Signal Processor)

**檔案位置**: `tradingagents/graph/signal_processing.py`
...
**檔案位置**: `tradingagents/graph/reflection.py`
...
#### Reflection Human Message Template
```
...
**檔案位置**: `tradingagents/dataflows/openai.py`

#### System Message (get_stock_news_openai)
```
Can you search Social Media for {query} from {start_date} to {end_date}? Make sure you only get the data posted during that period.
```
...
#### System Message (get_global_news_openai)
```
Can you search global or macroeconomics news from {look_back_days} days before {curr_date} to {curr_date} that would be informative for trading purposes? Make sure you only get the data posted during that period. Limit the results to {limit} articles.
```
...
#### System Message (get_fundamentals_openai)
```
...
- 所有提示詞均保持原始格式，未做任何修改
- 變數使用 `{variable_name}` 格式標示
- 部分提示詞包含動態變數，實際使用時會填入具體值
- 提示詞中的拼寫錯誤（如 "halluncinate"）均保持原樣
