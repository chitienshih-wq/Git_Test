...
flowchart TD
    ...
    style InitGraph fill:#1E90FF
    style Propagate fill:#FF8C00
```
...
flowchart TD
    ...
    style Start fill:#228B22
    style End fill:#C71585
    style InitLLM fill:#FF8C00
    style SetupGraph fill:#1E90FF
```
...
flowchart TD
    ...
    style Start fill:#228B22
    style End fill:#C71585
    style CheckAnalysts fill:#FF8C00
    style AddEdges fill:#1E90FF
```
...
flowchart TD
    ...
    StreamMode --> ProcessChunks[處理每個 chunk<br/>chunk.messages最後一條.pretty_print<br/>trace.append chunk]
    ProcessChunks --> GetFinalState1[取得最終狀態<br/>final_state = trace最後一個]
    ...
    style Start fill:#228B22
    style End fill:#C71585
    style CheckDebug fill:#FF8C00
    style ProcessSignal fill:#1E90FF
```
...
flowchart TD
    ...
    CreateState --> SetMessages[設定 messages<br/>列表包含 human 和 company_name]
    SetMessages --> SetCompany[設定 company_of_interest<br/>company_name]
    SetCompany --> SetTradeDate[設定 trade_date<br/>str trade_date]
    
    SetTradeDate --> CreateInvestDebateState[建立投資辯論狀態<br/>InvestDebateState<br/>history: 空字串<br/>current_response: 空字串<br/>count: 0]
    
    CreateInvestDebateState --> CreateRiskDebateState[建立風險辯論狀態<br/>RiskDebateState<br/>history: 空字串<br/>current_risky_response: 空字串<br/>current_safe_response: 空字串<br/>current_neutral_response: 空字串<br/>count: 0]
    
    CreateRiskDebateState --> InitReports[初始化報告欄位<br/>market_report: 空字串<br/>fundamentals_report: 空字串<br/>sentiment_report: 空字串<br/>news_report: 空字串]
...
flowchart TD
    ...
    PartialPrompt --> CreateChain[建立執行鏈<br/>chain = prompt 連接 llm.bind_tools tools]
    ...
    CheckToolCalls -->|否| EmptyReport[report = 空字串<br/>將由工具節點處理]
    
    SetReport --> Return
    EmptyReport --> Return
    
    Return[返回<br/>messages: result列表<br/>market_report: report] --> End([完成])
    
    style Start fill:#228B22
    style End fill:#C71585
    style CheckToolCalls fill:#FF8C00
    style CreateChain fill:#1E90FF
```
...
flowchart TD
    ...
    Return[返回<br/>messages: result列表<br/>sentiment_report: report] --> End([完成])
...
flowchart TD
    ...
    style Start fill:#228B22
    style End fill:#C71585
    style CheckSuccess fill:#FF8C00
    style CheckFallback fill:#FF8C00
```
...
flowchart TD
    ...
    style Start fill:#228B22
    style End fill:#C71585
    style CheckVendorSupported fill:#FF8C00
    style CheckResult fill:#FF8C00
```
...
flowchart TD
    ...
    InvokeLLM --> Return[返回<br/>messages: result列表<br/>trader_investment_plan: result.content]
...
flowchart TD
    ...
    style End fill:#C71585
    style InvokeLLM fill:#1E90FF
    style SetFinalDecision fill:#FF8C00
```
...
```mermaid
flowchart TD
    Start([should_continue_analyst state]) --> GetLastMessage[取得最後一條訊息<br/>last_message = messages最後一條]
...
flowchart TD
    ...
    style Start fill:#228B22
    style End fill:#C71585
    style CheckCount fill:#FF8C00
    style CheckLastSpeaker fill:#FF8C00
```
